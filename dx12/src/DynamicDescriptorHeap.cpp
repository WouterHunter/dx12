...
DynamicDescriptorHeap::DynamicDescriptorHeap(Device& device, D3D12_DESCRIPTOR_HEAP_TYPE heapType,
    ...
    m_DescriptorHandleIncrementSize = d3d12Device->GetDescriptorHandleIncrementSize(heapType);

    // Allocate space for staging CPU visible descriptors.
    m_DescriptorHandleCache = MakePtr<D3D12_CPU_DESCRIPTOR_HANDLE[]>(m_NumDescriptorsPerHeap);
}
...
void DynamicDescriptorHeap::ParseRootSignature(const RootSignature& rootSignature) {
    ...
    DWORD    rootIndex;
    while (_BitScanForward(&rootIndex, descriptorTableBitMask) && rootIndex < rootSignature.d3d12Desc.NumParameters) {
...
ComPtr<ID3D12DescriptorHeap> DynamicDescriptorHeap::CreateDescriptorHeap() {
    auto d3d12Device = m_Device.d3d12Device10;
...
void DynamicDescriptorHeap::CommitDescriptorTables(
    ...
    if (numDescriptorsToCommit > 0) {
        auto d3d12Device = m_Device.d3d12Device10;
        auto d3d12GraphicsCommandList = commandList.d3dCommandList.Get();
