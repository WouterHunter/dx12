...
void Texture::Resize(ivec2 size, u32 depthOrArraySize) {
    ASSERT(d3d12Resource);

    Device& device = m_Context->GetDevice();
    GlobalLayoutTracker& tracker = m_Context->GetGlobalLayoutTracker();

    // Unregister the texture resource
    tracker.Unregister(d3d12Resource.Get());

    // Retain the name of the resource if one was already specified.
    wchar_t nameBuffer[128] = {};
    UINT nameBufferLen = sizeof(nameBuffer);
    d3d12Resource->GetPrivateData(WKPDID_D3DDebugObjectNameW, &nameBufferLen, nameBuffer);

    // Create a texture resource with the new size
    CD3DX12_HEAP_PROPERTIES heapProps(D3D12_HEAP_TYPE_DEFAULT);
    CD3DX12_RESOURCE_DESC1 desc(d3d12Resource->GetDesc());
    desc.Width = (UINT64)std::max(size.x, 1);
    ...
    ThrowIfFailed(device.d3d12Device10->CreateCommittedResource3(
        ...
        &desc, D3D12_BARRIER_LAYOUT_COMMON, &m_ClearValue, nullptr, 0, nullptr,
        IID_PPV_ARGS(&d3d12Resource)));

    if (nameBuffer) d3d12Resource->SetName(nameBuffer);

    // Register the new resource
    tracker.Register(d3d12Resource.Get(), D3D12_BARRIER_LAYOUT_COMMON);
