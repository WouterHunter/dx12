...
namespace {
	...
	D3D12_RT_FORMAT_ARRAY GetRenderTargetFormats(Resource* resource) {
		D3D12_RESOURCE_DESC desc = resource->GetD3D12ResourceDesc();
	...
	void CopyResource(Context* context, CommandList* commandList, Resource* resource, ComPtr<ID3D12Resource>& uploadResource, size_t numElements,
		...
		if (bufferData) {
			...
			commandList->Transition(resource, RS_COMMON, RS_COPY_DEST);
...
static VertexPosColor TRIANGLE_VERTICES[] = {
	...
	{ .position = { 0.0f,  0.5f, -0.5f }, .color = { 0, 1, 0 } },
	{ .position = { 0.0f,  0.0f,  0.5f }, .color = { 0, 0, 1 } },
};
...
int CALLBACK wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR lpCmdLine, int) {
	...
	struct PipelineStateStream {
		...
		.rtvFormats = GetRenderTargetFormats(window->swapChain.GetBackBuffer()),
	};
